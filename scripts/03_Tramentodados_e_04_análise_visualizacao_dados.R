...
drive_auth()  # conta utilizada: trabalhor296@gmail.com
2
...
# 1) Garantir tipo numérico
dados_FC_classificados <- dados_FC_classificados %>%
  mutate(valor_remuneracao_media = as.numeric(valor_remuneracao_media))

#2) Definir teto 
#Diagnóstico da distribuição salarial
diag_ano <- dados_FC_classificados %>%
  ...
  summarise(
    ...
    p90     = quantile(valor_remuneracao_media, 0.90),
    p95     = quantile(valor_remuneracao_media, 0.95),
    p99     = quantile(valor_remuneracao_media, 0.99),
    p995    = quantile(valor_remuneracao_media, 0.995),
    max     = max(valor_remuneracao_media),
    .groups = "drop"
  )

diag_ano

#Quantos registros acima de R$ 200 mil em 2022?
salarios_200k <- dados_FC_classificados %>%
  filter(
    ano == 2022,
    !is.na(valor_remuneracao_media),
    valor_remuneracao_media >= 200000
  )

nrow(salarios_200k)

# Justificativa da definição do teto
#A inspeção da distribuição salarial indicou a presença de valores extremamente elevados concentrados exclusivamente no ano de 2022.
#Nesse ano, aproximadamente 9.129 vínculos apresentaram remuneração média anual superior a R$ 200.000, com valores máximos próximos a R$ 3 milhões.
#Nos demais anos da série analisada (2019, 2020, 2021 e 2023), os valores máximos observados ficaram abaixo de R$ 200.000 (cerca de R$ 149 mil, R$ 155 mil, R$ 164 mil e R$ 196 mil).
#Diante disso, adotou-se um teto absoluto de R$ 200.000 para a remuneração média, mitigando a influência de valores extremos e preservando a comparabilidade intertemporal.
#Adicionalmente, a mediana salarial foi utilizada como medida robusta de tendência central.

## salários com teto de R$ 200 mil para reduzir influência de valores extremos no setor FC.

TETO_SALARIO <- 200000

# 3) Base válida de salários + criação do salário com teto
...
# 3.1) Salário médio (bruto) x salário médio (com teto) + mediana — por ano
...
# 3.2) Deflacionar (IPCA base 2023)
...
# 5) GRÁFICO  - Salário por ANO x SEXO
# 5.1) Tabela 
salario_sexo_fc <- dados_fc_sal %>%
  group_by(ano, sexo) %>%
  summarise(
    n = n(),
    Media_Teto    = mean(salario_teto, na.rm = TRUE),
    Mediana_Bruta = median(salario_bruto, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  left_join(fatores_inflacao, by = "ano") %>%
  mutate(
...
print(salario_sexo_fc)

# 5.2) Formato longo para plotar Média x Mediana no mesmo gráfico
...
ggplot(
  ...
  labs(
    ...
    subtitle = "Média com teto (R$ 200 mil) e mediana — valores reais (base 2023)",
    x = "Ano",
    y = "Salário real (R$)",
    color = "Sexo",
    linetype = "Medida"
...
ggsave("salario_real_fc_media_teto_e_mediana_por_sexo_2019_2023.png",  width = 10, height = 6, dpi = 300)


# 6) GRÁFICO  - Salário por ANO x CLASSIFICAÇÃO (nominal -> real base 2023)
# 6.1) Tabela 

salario_classe_nominal <- dados_fc_sal %>%
  filter(!is.na(CLASSIFICAÇÃO)) %>%
  group_by(ano, CLASSIFICAÇÃO) %>%
  summarise(
    n = n(),
    Media_Teto_Nominal = mean(salario_teto, na.rm = TRUE),
    Mediana_Nominal    = median(salario_bruto, na.rm = TRUE),
    .groups = "drop"
  )

salario_classe_fc <- salario_classe_nominal %>%
  left_join(fatores_inflacao, by = "ano") %>%
  mutate(
    Media_Teto_Real = Media_Teto_Nominal / fator_ipca,
    Mediana_Real    = Mediana_Nominal    / fator_ipca
  ) %>%
  arrange(ano, CLASSIFICAÇÃO)

print(salario_classe_fc)
...
ggsave("salario_real_fc_media_teto_por_nivel_hierarquico_2019_2023.png",width = 10, height = 6, dpi = 300)


#7) TABELA TOP 5 - Top 5 Maiores e Menores Salários Médios por Ocupação (2023)
#   (média com teto R$ 200 mil)

#7.1) 
ANO_ANALISE <- 2023
MIN_VINCULOS_OCUP <- 50
...
salario_ocupacao_2023 <- dados_fc_sal %>%
  ...
  filter(Total_Vinculos >= MIN_VINCULOS_OCUP) %>%
...
#7.3)Top 5 menores (aqui estava o erro no seu arrange)
top5_menores_fc <- salario_ocupacao_2023 %>%
  arrange(Salario_Medio_Teto) %>%
  slice_head(n = 5) %>%
  transmute(
    `CBO - Ocupações` = OCUPAÇÕES,
    Categoria = CLASSIFICAÇÃO,
...
tabela_top5_fc_gt <- tabela_top5_fc %>%
  ...
  tab_spanner(label = glue("5 maiores salários médios {ANO_ANALISE}"),
              columns = ends_with("(Top 5)")) %>%
  tab_spanner(label = glue("5 menores salários médios {ANO_ANALISE}"),
              columns = ends_with("(Bottom 5)")) %>%
  fmt_currency(columns = matches("Salário médio"),
               currency = "BRL",
               decimals = 2) %>%
  tab_source_note(source_note = "Fonte: RAIS. Elaboração própria.") %>%
...
#Exportar para Excel
write_xlsx(tabela_top5_fc, glue("tabela_top5_maiores_menores_salarios_fc_2023.xlsx"))


# 8)TABELA FINAL  - Salário por Ano x UF x Macrorregião x Sexo
...
tabela_trabalhadores_fc <- dados_fc_sal %>%
  ...
  summarise(
    ...
    Salario_Medio_Teto  = mean(salario_teto, na.rm = TRUE),
    Mediana_Salario     = median(salario_bruto, na.rm = TRUE),
    .groups = "drop"
  )%>%
  left_join(fatores_inflacao, by = "ano") %>%
  mutate(
